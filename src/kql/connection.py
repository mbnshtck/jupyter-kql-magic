...
class Connection(object):
    ...
    def _get_kusto_database_engine(cls, conn_name):
        ...
        if len(parts) != 2:
            raise KqlEngineError('invalid connection_str, valid connection_str patternes are: "kusto://...", "appinsights://...", "loganalytics://..." and "database@cluster"')
        if parts[1] in ['appinsights','loganalytics']:
            raise KqlEngineError('invalid connection_str, connection_str pattern "database@cluster" cannot be used for "appinsights" and "loganalytics"')
        cluster_conn_name = '@' + parts[1]
        cluster_conn = cls.connections.get(cluster_conn_name)
        if cluster_conn is None:
            raise KqlEngineError('invalid connection_str, connection_str pattern "database@cluster" can be used only after a connection_str pattern "kusto://..." to the same cluster was previously defined')
