...
class Palette(list):
    ...
    def __init__(self, palette_name=None, n_colors=None, desaturation=None, rgb_palette=None, range_start=None, to_reverse=False, **kwargs):
        ...
        if rgb_palette is None:
            ...
            if rgb_palette is None:
                sns_pallete = sns.color_palette(palette = parsed.get('base_name'), 
                                                n_colors = self.n_colors,
                                                desat = self.desaturation)
    ...
    def _to_html(self, add_details_to_name=True):
        ...
        if add_details_to_name:
            desaturation_details = ', desaturation {0}'.format(self.desaturation) if self.desaturation is not None and self.desaturation > 0  and self.desaturation < 1.0 else ''
    ...
    def validate_palette_name(cls, name):
        ...
        if parsed.get('rgb_palette') is None and  parsed.get('base_name') not in Palettes.BASE_PALETTE_NAMES:
            raise AttributeError('must be a known palette name or custom palette (see option -popup_palettes) , but a value of {0} was specified.'.format(name))
